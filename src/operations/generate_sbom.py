...
def build_sbom(packages: list[str] | None = None, application_version: str | None = None) -> dict[str, object]:
    ...
    for package in packages or DIRECT_PACKAGES:
        ...
        component = {"type": "library", "bom-ref": f"pkg:pypi/{normalized}@{package_version}", "name": package_metadata.get("Name", package), "version": package_version, "purl": f"pkg:pypi/{normalized}@{package_version}"}
